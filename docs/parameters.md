...
| ----------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `design_mode`     | null            | Pipeline mode. Choose from: `monomer_denovo`, `monomer_foldcond`, `monomer_motifscaff`, `monomer_partialdiff`, `bindcraft_denovo`, `binder_denovo`, `binder_foldcond`, `binder_motifscaff`, `binder_partialdiff`, `boltzgen_denovo`, or `boltzgen_redesign` |
...
| --------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `design_length`                   | null    | Design length of binder or monomer. e.g. `'60'` or `'60-70'`. Pipeline will randomly sample different sizes between these values.                                                                                                                                                                                                 | `bindcraft_denovo`, `binder_denovo`, `monomer_denovo`, `boltzgen_denovo`                                                                      |
| `input_pdb`                       | null    | Path to input PDB file (e.g. target for binders, `'./target.pdb'`). Required for several modes.                                                                                                                                                                                                                                   | `binder_denovo`, `binder_foldcond`, `binder_motifscaff`, `binder_partialdiff`, `monomer_motifscaff`, `monomer_partialdiff`, `boltzgen_denovo`, `boltzgen_redesign` |
...
| --------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `bg_not_binding_residues`   | null    | Target residues the binder should NOT contact. Comma-separated tokens, each a chain-qualified residue e.g. `'A200'`, a chain-qualified range e.g. `'A210-215'`, or a bare chain ID e.g. `'B'` meaning the whole chain.                                                                                | `boltzgen_denovo`, `boltzgen_redesign` |
| `bg_redesign_spec`          | null    | Chain A (binder) architecture spec: an ordered, comma-separated list of chain-A keep tokens (`'A<start>-<end>'` or `'A<n>'`) and bare-digit insert tokens (`'<n>'` or `'<min>-<max>'`), e.g. `'7-10,A1-60,5,A70-100,10'` inserts 7-10 residues at the N-terminus, keeps A1-60, inserts 5 residues, keeps A70-100, then appends 10 residues at the C-terminus. Residues not covered by a keep token are deleted. If null, chain A's architecture (length) is left unchanged. At least one of `bg_redesign_spec`/`bg_redesign_inpaint_seq`/`bg_flexible_residues` must be set. | `boltzgen_redesign`  |
| `bg_redesign_inpaint_seq`   | null    | Chain A residues, within those kept by `bg_redesign_spec`, whose sequence is allowed to change while structure stays fixed/conditioned (unless also in bg_flexible_residues), comma-separated e.g. `'A10-50,A60'`. Must only reference chain A residues not deleted by `bg_redesign_spec`.                                                                                                                                                                  | `boltzgen_redesign`  |
| `bg_flexible_residues`      | null    | Residues whose structure should NOT be conditioned on, useful for disordered/flexible regions e.g. loops or IDPs. Comma-separated tokens, each a chain-qualified residue e.g. `'A16'`, a chain-qualified range e.g. `'B10-13'`, or a bare chain ID e.g. `'C'` meaning the whole chain. In `boltzgen_redesign` mode this may also reference chain A residues kept by `bg_redesign_spec`.  | `boltzgen_denovo`, `boltzgen_redesign` |
...
| `af2_jax_compilation_cache_dir` | null | Optional existing path for the persistent JAX compilation cache used by AlphaFold2. The path is bind-mounted into the container. When null, `JAX_COMPILATION_CACHE_DIR` is not set. |
